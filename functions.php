<?php
	...
	add_action('wp_enqueue_scripts', 'citron_css');

	// theme setup
	function citron_setup(){
		// navgation menu
		register_nav_menus(
			array(
				'primary' => __('Citron Menu')
			)
		);
		// add featured image support
		add_theme_support('post-thumbnails');
		add_image_size('water-thumbnail', 990, 495, true);
		add_image_size('typical-thumbnail', 526, 804, true);
		add_image_size('drinking-thumbnail', 330, 280, ture);
		// add post format support
		add_theme_support('post-formats', array('gallery','link'));
	}
	add_action('after_setup_theme', 'citron_setup');
